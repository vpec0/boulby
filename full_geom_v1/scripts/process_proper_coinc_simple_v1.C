 ...
 *
 * Skin, GdLS and WT are searched for anti-coincidence
 *
 * TPC depositions collected over 1 ms
 * Veto depositions collected over 1 us
 * Veto signals recorded within -1 s and 10
 **************************************************************/
...
void testOtherDep(deposition_vector&,  deposition_vector&, double, double, double, double);
void printDepositions(deposition_vector& deps);
void printDeposition(deposition_t& dep);


void process_proper_coinc_simple_v1(const char* basedir = "data/full_geom_v0_4classes",
				    int batchNo = 4, int Nruns = 10,
				    const char* outfname = "", int startrun = 0)
{

    // input tree
    auto tree = new TChain("events");
    size_t size = attachFiles(tree, "", batchNo, Nruns, startrun, basedir);
	...
	const double before = 1e6; // veto window before signal; us
	const double after = 1e3; // veto window after signal
	for (size_t idep = 0; idep < tpc_deps[0].size(); idep++) {
	    ...
	    deposition_vector depSkin;
	    deposition_vector depGdls;
	    deposition_vector depWt;

	    testOtherDep(skin_deps, depSkin, tpc_dep.T1, tpc_dep.T2, before, after);
	    testOtherDep(gdls_deps, depGdls, tpc_dep.T1, tpc_dep.T2, before, after);
	    testOtherDep(wt_deps, depWt, tpc_dep.T1, tpc_dep.T2, before, after);
	    ...
	    // store all veto
	    int i = 0;
	    for (auto& veto: depSkin) {
		if (i >= ProperCoincTree::MAX_VETO_DEPS) break;
		outevt->veto_deps[0][i] = veto.E;
		outevt->veto_t1[0][i] = veto.T1;
		outevt->veto_t2[0][i] = veto.T2;
		i++;
	    }
	    outevt->n_veto_deps[0] = i;

	    i = 0;
	    for (auto& veto: depGdls) {
		if (i >= ProperCoincTree::MAX_VETO_DEPS) break;
		outevt->veto_deps[1][i] = veto.E;
		outevt->veto_t1[1][i] = veto.T1;
		outevt->veto_t2[1][i] = veto.T2;
		i++;
	    }
	    outevt->n_veto_deps[1] = i;

	    i = 0;
	    for (auto& veto: depWt) {
		if (i >= ProperCoincTree::MAX_VETO_DEPS) break;
		outevt->veto_deps[2][i] = veto.E;
		outevt->veto_t1[2][i] = veto.T1;
		outevt->veto_t2[2][i] = veto.T2;
		i++;
	    }
	    outevt->n_veto_deps[2] = i;
...
void testOtherDep(deposition_vector& deps, deposition_vector& to_save,
		  double t1, double t2, double before, double after)
{
    // see if any depositions within the time before and after the T1 and T2, respectively
    for (auto& dep: deps) {
	if ( dep.T1 < (t2+after) && dep.T2 > (t1-before) ) {
	    to_save.push_back(dep);
	}
    }
