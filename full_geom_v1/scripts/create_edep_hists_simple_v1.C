...
void create_edep_hists_simple_v1(const char* fname, const char* outpref = "")
{
    ...
    // output file
    auto outf = TFile::Open(Form("%sedep_hists.root", outpref), "recreate");
    ...
    vector<TH1*> hist_vec;
    TString hname = "hEdepXeTPC";
    TH1* h = 0;
    h = new TH1D(hname ,";Deposited energy [MeV]", 400, 1e-4, 1e4); hist_vec.push_back(h);
    doXlog(h);
    h = (TH1*)h->Clone( Form("%s_fullveto", (const char*)hname) ); hist_vec.push_back(h);
    h = (TH1*)h->Clone( Form("%s_noother", (const char*)hname) ); hist_vec.push_back(h);
    h = (TH1*)h->Clone( Form("%s_noother_fullveto", (const char*)hname) ); hist_vec.push_back(h);
    h = (TH1*)h->Clone( Form("%s_fullveto_nols", (const char*)hname) ); hist_vec.push_back(h);
    h = (TH1*)h->Clone( Form("%s_noother_fullveto_nols", (const char*)hname) ); hist_vec.push_back(h);

    hname = "hEdepAllTPC";
    h = (TH1*)h->Clone( hname ); hist_vec.push_back(h);
    h = (TH1*)h->Clone( Form("%s_fullveto", (const char*)hname) ); hist_vec.push_back(h);
    // h = (TH1*)h->Clone( Form("%s_fullveto_nols", (const char*)hname) ); hist_vec.push_back(h);
	...
	auto full_Edep = evt.tpc_deps[0] + evt.tpc_deps[1] + evt.tpc_deps[2] + evt.tpc_deps[3];
	...
	// fill hists
	hists[kFull]->Fill(evt.tpc_deps[0]);
	hists[kAllDepsFull]->Fill(full_Edep);

	if (other_deps)
	    hists[kNoOthers]->Fill(evt.tpc_deps[0]);
	if (veto[2] && veto[1] && veto[0]) {
	    hists[kFullVeto]->Fill(evt.tpc_deps[0]);
	    hists[kAllDepsVeto]->Fill(full_Edep);
	}
	if (other_deps && veto[2] && veto[1] && veto[0])
	    hists[kNoOthersFullVeto]->Fill(evt.tpc_deps[0]);
